...
void scriviMostre(struct mostre *testa) {
    ...
    for (temp = testa; temp != NULL; temp = temp->nextMostra) {
        ...
        if (size == 0) { //file vuoto.
            fprintf(fp, "%d,%s,%s,%s,%s,%d", temp->id, temp->citta, temp->indirizzo, temp->dataInizio, temp->dataFine, temp->nOpere);
        } else { //file pieno
            fprintf(fp, "\n%d,%s,%s,%s,%s,%d", temp->id, temp->citta, temp->indirizzo, temp->dataInizio, temp->dataFine, temp->nOpere);
        }
    }
