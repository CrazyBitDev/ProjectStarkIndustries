...
void scriviOpere(Opera *testa) {
    ...
    for (temp = testa; temp != NULL; temp = temp->nextOpera) {
        ...
        if (size == 0) {
            //file vuoto
            fprintf(fp, "%d,%s,%s,%s,%s,%s,%d", temp->id, temp->nome, temp->autore, temp->tipo, temp->genere,
                    temp->periodo, temp->anno);
        } else {
            //file vuoto
            fprintf(fp, "\n%d,%s,%s,%s,%s,%s,%d", temp->id, temp->nome, temp->autore, temp->tipo, temp->genere,
                    temp->periodo, temp->anno);
        }
    }
    fclose(fp);
 ...
 *
 *   @return opera ricercata, altrimenti NULL
 */
 ...
 *
 *   @return se selezione == true l'opera selezionata, altrimenti NULL
 */
